...
$submit = isset($HTTP_POST_VARS['equip']);
...
if ( $user_id == $searchid )
{
	...
	$equip_lgreave = $lang['Adr_battle_select_greave'];
	$equip_lboot = $lang['Adr_battle_select_boot'];
...
else
{
	...
	$equip_lgreave = $lang['Adr_equip_greave'];
	$equip_lboot = $lang['Adr_equip_boot'];
	...
	$greave_list = $lang['Adr_battle_no_greave'];
	$boot_list = $lang['Adr_battle_no_boot'];
	...
	for ( $i = 0 ; $i < count($items) ; $i ++ )
	{
		...
		if ( $items[$i]['item_type_use'] == 29 && $items[$i]['item_id'] == $equip_greave )
		{
			$greave_list = adr_get_lang($items[$i]['item_name']) . ' ( ' . $lang['Adr_items_power'] . ' : ' . $items[$i]['item_power'] . ' - ' . $lang['Adr_items_duration'] . ' : ' . $items[$i]['item_duration'] . ' )';
			$greave_pic = $items[$i]['item_icon'];
		}


		if ( $items[$i]['item_type_use'] == 30 && $items[$i]['item_id'] == $equip_boot )
		{
			$boot_list = adr_get_lang($items[$i]['item_name']) . ' ( ' . $lang['Adr_items_power'] . ' : ' . $items[$i]['item_power'] . ' - ' . $lang['Adr_items_duration'] . ' : ' . $items[$i]['item_duration'] . ' )';
			$boot_pic = $items[$i]['item_icon'];
		}
...
$template->assign_vars(array(
	...
	'L_SELECT_GREAVE' => $equip_lgreave,
	'L_SELECT_BOOT' => $equip_lboot,
