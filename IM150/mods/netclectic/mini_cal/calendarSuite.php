...
function makeTimestamp($date) {
	// bug fix for +1 month on 31/03 ==> says 01/05 !
	if(strstr($date,'month')){
		// 31/03 + 1 month = 1/05 !!! not 30/04
		// use mid month for calculating # days next month
		$midmonth = strtotime(date('Y-m-15').' '.$date);
		$midsmonthdays = date("t",$midmonth);
		if(date("d") > $midsmonthdays){
			$newdate = strtotime(date('Y-m-'.$midsmonthdays).' '.$date);
		} else {
			$newdate = strtotime($date);
		}
		// timezone incorporation
		board2usertime($newdate);
		$this->stamp = $newdate;
	} else {
