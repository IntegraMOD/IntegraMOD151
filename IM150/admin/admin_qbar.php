...
$ct_ignorepvar = array('url', 'name', 'explain', 'shortcut', 'alternate');
...
function select_field($import=false)
{
	...
	while ( list($qbar_name, $qbar) = @each($qbar_maps) )
	{
		...
		if (($panel_id != 0) || isset($open_ids[$i]))
		{
			if ((($panel_id == 0) || ($panel_id == $i)) && ($qbar_name == 'default_tree'))
			{
				...
				{
					if ($field['level'] > $max_level) $max_level = $field['level'];
				}
			}
		}
	}
	if (!$is_default) $max_level = 0;
	...
	while ( list($qbar_name, $qbar) = @each($qbar_maps) )
	{
		...
		if ( empty($panel_id) || ($panel_id == $i) )
		{
			...
			$template->assign_block_vars('qbar', array(
				'PANEL_ID'		=> $i,
				'NAME'			=> $name,
				'CLASS'			=> $qbar['class'],
			...
			if ( !empty($panel_id) || isset($open_ids[$i]) )
			{
				...
				while ( list($field_name, $field) = @each($qbar['fields']) )
				{
					...
					$template->assign_block_vars('qbar.field', array(
					...
					if ($qbar_name == 'default_tree')
					{
						...
						{
							$color_inc = !$color_inc;
							$template->assign_block_vars('qbar.field.inc', array(
...
if (($mode == 'edit') && (!empty($field_id) || $create_field))
{
	...
	if ($mode == 'edit')
	{
		...
		else if ($submit)
		{
			...
			if (ereg("http", strtolower($field_url)) && $field_internal)
			{
				$error = true;
				$error_msg .= (($error_msg != '') ? '<br />' : '') . $lang['Qbar_error_field_empty_url'];
			}
...
if (($mode == 'edit') && empty($field_id) && !$create_field)
{
	...
	else
	{
		...
		reset($classes);
		while (list($key, $lang_key) = each($classes)) if ($key != 'System')
		{
