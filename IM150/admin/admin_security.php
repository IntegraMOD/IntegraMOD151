	...
	if (!$allowed)
		...
		while ($daily_backups = @readdir($browse)) 
			...
			if ( ($daily_backups != '.htaccess') && ($daily_backups != '.') && ($daily_backups != '..') && ($daily_backups != 'index.htm') )
				{
			$a++;
			$backups .= '<br>'. $a .'. <a href="'. append_sid('admin_security.'. $phpEx .'?mode=unlink&amp;file='. $daily_backups) .'">'. $daily_backups .'</a> :: '. AdminBackupSize(@filesize($backups_dir . $daily_backups)) .' :: '. create_date($board_config['default_dateformat'], @filemtime($backups_dir . $daily_backups), $board_config['board_timezone']);
				}
			}
