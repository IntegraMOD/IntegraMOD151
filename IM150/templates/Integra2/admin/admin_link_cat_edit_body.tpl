<h1>{L_DL_CAT_TITLE}</h1>

<p>{L_DL_CAT_EDIT_TEXT}</p>

<form action="{S_CATEGORY_ACTION}" method="post"><table class="forumline" cellpadding="4" cellspacing="1" border="0" align="center">
   <tr>
      <th class="thTop" colspan="2">{L_DL_CAT_TITLE}</th>
   </tr>
   <tr>
      <td class="row1" width="38%"><span class="gen">{L_DL_NAME}:</span></td>
      <td class="row2"><input type="text" name="cat_title" size="40" maxlength="100" value="{CAT_NAME}" /></td>
   </tr>
   <tr>
      <td class="cat" colspan="2" align="center"><input type="submit" name="submit" value="{L_SUBMIT}" class="mainoption" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" value="{L_RESET}" class="liteoption" /></td>
   </tr>
</table>
{S_HIDDEN_FIELDS}</form> 
<div align="center"><span class="copyright">Links MOD v1.2.1 by <a href="http://www.phpbb2.de" target="_blank">phpBB2.de</a> and OOHOO</span></div>