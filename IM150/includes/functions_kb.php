 ...
 * 
 *    $Id: functions_kb.php,v 1.32 2005/04/21 19:36:17 jonohlsson Exp $
 */
   ...
   $sql = "SELECT t.article_id, t.article_category_id, t.article_title, t.article_description, t.article_date, t.article_author_id, t.username, t.article_type, t.approved, t.views, t.article_rating, t.article_totalvotes, u.user_id, u.user_rank, u.user_sig, u.user_sig_bbcode_uid, u.user_allowsmile
	...
	while ( $article = $db->sql_fetchrow( $article_result ) )
	{
		...
		if ( defined( 'IN_ADMIN' ) )
		{
			...
			{ 
				// approve
				$temp_url = $module_root_path . "admin/admin_kb_art.$phpEx?mode=approve&amp;a=$article_id" . "&amp;start=" . $start. "&amp;sid=".$userdata['session_id'];
				$approve = '<a href="' . $temp_url . '"><img src="' . $images['icon_approve'] . '" border="0" alt="' . $lang['Approve'] . '"></a>';
			}
			elseif ( $article_approved == 1 )
			{ 
				// unapprove
				$temp_url = $module_root_path . "admin/admin_kb_art.$phpEx?mode=unapprove&amp;a=$article_id" . "&amp;start=" . $start. "&amp;sid=".$userdata['session_id'];
				$approve = '<a href="' . $temp_url . '"><img src="' . $images['icon_unapprove'] . '" border="0" alt="' . $lang['Un_approve'] . '"></a>';
			} 
			// delete
			$temp_url = $module_root_path . "admin/admin_kb_art.$phpEx?mode=delete&amp;a=$article_id" . "&amp;start=" . $start. "&amp;sid=".$userdata['session_id'];
...
function get_kb_stats( $type = false, $approve, $block_name, $start = -1, $articles_in_cat = 0, $kb_is_auth )
{
	...
	$sql = "SELECT article_id, article_category_id, article_title, article_description, article_date, article_author_id, username, article_type, approved, views, article_rating, article_totalvotes
    FROM " . KB_ARTICLES_TABLE . " WHERE";
	...
	while ( $article = $db->sql_fetchrow( $article_result ) )
	{
		...
		if ( defined( 'IN_ADMIN' ) || $userdata['user_level'] == ADMIN )
		{
			...
			{ 
				// approve
				$temp_url = $module_root_path . "admin/admin_kb_art.$phpEx?mode=approve&amp;a=$article_id" . "&amp;sid=".$userdata['session_id'];
				$approve = '<a href="' . $temp_url . '"><img src="' . $images['icon_approve'] . '" border="0" alt="' . $lang['Approve'] . '"></a>';
			}
			elseif ( $article_approved == 1 )
			{ 
				// unapprove
				$temp_url = $module_root_path . "admin/admin_kb_art.$phpEx?mode=unapprove&amp;a=$article_id" . "&amp;sid=".$userdata['session_id'];
				$approve = '<a href="' . $temp_url . '"><img src="' . $images['icon_unapprove'] . '" border="0" alt="' . $lang['Un_approve'] . '"></a>';
			} 
			// delete
			$temp_url = $module_root_path . "admin/admin_kb_art.$phpEx?mode=delete&amp;a=$article_id" . "&amp;sid=".$userdata['session_id'];
			$delete = '<a href="' . $temp_url . '"><img src="' . $phpbb_root_path . $images['icon_delpost'] . '" border="0" alt="' . $lang['Delete'] . '"></a>';
		}
...
{
	global $lang, $board_config, $kb_config, $db, $module_root_path, $phpbb_root_path, $mx_root_path, $phpEx, $userdata;
   ...
   if ( !empty($message) )
   {
      if ( !$error )
      {
		...
		$find = array('\n',
			...
			'[/url]',
			']');
			$privmsg_message = str_replace ($find, '', $message); 
      }
   }
   else
   {
...
{
	global $db, $template, $phpbb_root_path, $mx_root_path, $module_root_path, $phpEx, $is_block, $page_id, $kb_config, $userdata, $kb_quick_nav;
	...
	{ 
		...
		if ( MXBB_MODULE && !MXBB_27x )
		{
			...
			$sql_num = "SELECT count(article_id) AS total
				FROM " . KB_ARTICLES_TABLE . "
				WHERE "; 
			
			if ( $kb_config['news_operate_mode'] )
			{
				$kb_types_list = ns_auth_item( $category['category_id'] );
				$sql_num .= " article_type IN " . $kb_types_list . ' AND';
			}
			$sql_num .= " article_category_id = " . $category['category_id'] . " AND approved = '1'";
		...
		{
			$template->assign_block_vars( 'catrow', array( 'CATEGORY' => $category,
...
{
	global $db, $template, $phpbb_root_path, $mx_root_path, $module_root_path, $phpEx, $is_block, $page_id, $kb_config;
	...
	{ 
		...
		{		
			...
			$sql_num .= " article_category_id = " . $category['category_id'];
		...
		if ( ns_auth_cat( $category_id ) && $kb_is_auth_all[$category_id]['auth_view'] )
		{
			$template->assign_block_vars( 'switch_sub_cats.catrow', array( 'CATEGORY' => $category,
					'CAT_DESCRIPTION' => $category_details,
					'CAT_ARTICLES' => $category_articles ) 
				);
		}
...
function get_kb_cat_subs_admin( $parent, $select = 1, $indent, $ss )
{
	...
	while ( $category2 = $db->sql_fetchrow( $result ) )
	{
		...
		$template->assign_block_vars( 'catrow.subrow', array( 'CATEGORY' => $category2,
    ...
    $smilies_on = 1 )
{
	...
	if ( $mode == 'newtopic' || $kb_config['bump_post'] == '1' )
	{
      ...
      $sql = "INSERT INTO " . POSTS_TEXT_TABLE . " (post_id, post_subject, bbcode_uid, post_text) VALUES ($post_id, '$subject', '$bbcode_uid', '$message_tmp')";
	  if ( !$db->sql_query( $sql, BEGIN_TRANSACTION ) )
		{
			$error_die_function( GENERAL_ERROR, 'Error in posting', '', __LINE__, __FILE__, $sql );
		} 
...
function get_kb_comments( $topic_id = '', $start = -1, $show_num_comments = 0 )
{
	...
	for( $i = $i_init; $i < $total_posts; $i++ )
	{
		...
		if ( $board_config['allow_smilies'] )
		{
			...
			{
				$message = mx_smilies_pass( $message );
			}
		}
		$message = str_replace( "\n", "\n<br />\n", $message ); 
		...
		$template->assign_block_vars( 'postrow', array( 'POSTER_NAME' => $poster,
...
{
	global $news_type_select_data, $kb_type_select_data, $kb_config;
	...
	$tmp_kb = $kb_type_select_data[$cat_id]['forum_news'] == 1;
	$tmp_news = true;
	...
	return $tmp_kb && $tmp_news;
}

function ns_auth_item( $cat_id, $item_type = 0 )
{
	global $news_type_select_data, $kb_config;
	...
	if ( in_array( $item_type, $validated_types ) || !$kb_config['news_operate_mode'] )
	{
		return true;
	}
	else
