...
// Image Resize :: Added :: Start
function get_img_size_format($width, $height)
{
	get_img_size($width, $height, 1);
}
function get_img_size($width, $height, $thumbtype)
{
	...
	switch ($thumbtype){
		...
		case 2:
			// normal image
			$max_width = $attach_config['img_max_width'];
			$max_height = $attach_config['img_max_height'];
			break;
		default:
			$max_width = 0;
			$max_height = 0;
	...
	if ($max_width > 0 && $newwidth > $max_width){
		$newheight = round($newheight * ($max_width / $newwidth));
		$newwidth = round($newwidth * ($max_width / $newwidth));
	}
	if ($max_height > 0 && $newheight > $max_height){
		$newwidth = round($newwidth * ($max_height / $newheight));
		$newheight = round($newheight * ($max_height / $newheight));
	}
	return array(
		$newwidth,
		$newheight
...
/* Image Resize :: Altered
function create_thumbnail($source, $new_file, $mimetype) */
function create_thumbnail($source, $new_file, $mimetype, $thumbtype=1) 
{
	...
	list($new_width, $new_height) = get_img_size($width, $height, $thumbtype);
