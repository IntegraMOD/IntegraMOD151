...
if ($mode == '')
{
	...
	$template->assign_vars(array(
		...
		'APPROVAL_BUTTON' => ( ($userdata['user_level'] != ADMIN) && ($thiscat['cat_approval'] == ALBUM_ADMIN) ) ? '' : '<input type="submit" class="liteoption" name="approval" value="'. $lang['Approve'] .'" />',

		'UNAPPROVAL_BUTTON' => ( ($userdata['user_level'] != ADMIN) && ($thiscat['cat_approval'] == ALBUM_ADMIN) ) ? '' : '<input type="submit" class="liteoption" name="unapproval" value="'. $lang['Unapprove'] .'" />',
...
else
{
	...
	if ($mode == 'move')
	{
		...
		if( !isset($_POST['target']) )
		{
			...
			$category_select  .= album_get_simple_tree_option($cat_id, ALBUM_AUTH_MODERATOR);
