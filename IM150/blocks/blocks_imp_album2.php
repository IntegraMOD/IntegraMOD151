...
if(!function_exists(imp_album_block_func))
{
	function imp_album_block_func()
	{
		...
		$sql = "SELECT c.*, COUNT(p.pic_id) AS count 
						...
						".(($portal_config['md_pics_all2'] == '1') ? '' : 'WHERE cat_id <> 0')." 
						GROUP BY cat_id 
		...
		if ( $portal_config['md_pics_all2'] == '1' )
		{
		...
		{
			$CategoryID = $portal_config['md_cat_id2'];

			if ( $portal_config['md_pics_sort2'] == '1' )
			{
				if ( $CategoryID != 0 )
				{
					$sql = "SELECT p.pic_id, p.pic_title, p.pic_desc, p.pic_user_id, p.pic_user_ip, p.pic_username, p.pic_time, p.pic_cat_id, p.pic_view_count, u.user_id, u.username, r.rate_pic_id, AVG(r.rate_point) AS rating, COUNT(DISTINCT c.comment_id) AS comments
						...
						GROUP BY p.pic_id
						ORDER BY RAND()
						LIMIT ". $portal_config['md_pics_number2'];
				}
				else
				{
					$sql = "SELECT p.pic_id, p.pic_title, p.pic_desc, p.pic_user_id, p.pic_user_ip, p.pic_username, p.pic_time, p.pic_cat_id, p.pic_view_count, u.user_id, u.username, r.rate_pic_id, AVG(r.rate_point) AS rating, COUNT(DISTINCT c.comment_id) AS comments
			...
			else if ( $portal_config['md_pics_sort2'] == '0' )
			{
				if ( $CategoryID != 0 )
				{
					$sql = "SELECT p.pic_id, p.pic_title, p.pic_desc, p.pic_user_id, p.pic_user_ip, p.pic_username, p.pic_time, p.pic_cat_id, p.pic_view_count, u.user_id, u.username, r.rate_pic_id, AVG(r.rate_point) AS rating, COUNT(DISTINCT c.comment_id) AS comments
			...
			if (count($recentrow) > 0)
			{
				...
				{
					$template->assign_block_vars('recent_pics2', array());
					...
					for ($j = $i; $j < ($i + $album_config['cols_per_page']); $j++)
					{
						...
						$template->assign_block_vars('recent_pics2.recent_detail', array(
						...
						$template->assign_block_vars('recent_pics2.recent_detail2', array(
		...
		$template->assign_vars(array(
			...
			'L_NO_PICS2' => $lang['No_Pics2'],
			'L_PIC_TITLE' => $lang['Pic_Title'],
			'L_VIEW' => $lang['View'],
			'L_POSTER' => $lang['Poster'],
			'L_POSTED' => $lang['Posted'],
			'L_PICS_NUMBER' => $portal_config['md_pics_number2']
			// End add - Photo Album Block
			)
		);
