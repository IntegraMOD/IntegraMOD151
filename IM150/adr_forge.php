...
adr_template_file('adr_forge_body.tpl');
...
if ( $mode != "" )
{
	switch($mode)
	{
        ...
        {	
          ...
          $item = drop_gather_loot($actual_zone, $user_id, 'mining', 3);
          ...
          include($phpbb_root_path . 'adr/includes/adr_header.'.$phpEx);
          $direction = append_sid("adr_forge.$phpEx?mode=mining");
		...
		case 'repair' :
			...
			// weap prof : 40 , 41 , 42 , 43 , 44 , 45 , 46
			$sql = " SELECT * FROM " . ADR_SHOPS_ITEMS_TABLE . "
				WHERE item_owner_id = $user_id
				AND item_in_shop = 0
				AND item_in_warehouse = 0
				...
				AND item_type_use IN ( 5 , 6 , 7 , 8 , 9 , 29 , 30 , 10, 40 , 41 , 42 , 43 , 44 , 45 , 46) ";
			if ( !($result = $db->sql_query($sql)))
			{
		...
		case 'repair_action' :
			...
			$direction = append_sid("adr_forge.$phpEx?mode=repair");
			$message .= '<br /><br />'.sprintf($lang['Adr_return'],"<a href=\"" . $direction . "\">", "</a>") ;
			message_die ( GENERAL_MESSAGE , $message );
		...
		case 'recharge_action' :
			...
			$direction = append_sid("adr_forge.$phpEx?mode=recharge");
			$message .= '<br /><br />'.sprintf($lang['Adr_return'],"<a href=\"" . $direction . "\">", "</a>") ;
			message_die ( GENERAL_MESSAGE , $message );
		...
		case 'stone_action' :
			...
			$direction = append_sid("adr_forge.$phpEx?mode=stone");
			$message .= '<br /><br />'.sprintf($lang['Adr_return'],"<a href=\"" . $direction . "\">", "</a>") ;
			message_die ( GENERAL_MESSAGE , $message );
		...
		case 'enchant_action' :
			...
			$direction = append_sid("adr_forge.$phpEx?mode=enchant");
			$message .= '<br /><br />'.sprintf($lang['Adr_return'],"<a href=\"" . $direction . "\">", "</a>") ;
			message_die ( GENERAL_MESSAGE , $message );
...
$template->assign_vars(array(
	...
	'U_REPAIR_ITEM' => append_sid("adr_forge.$phpEx?mode=repair"),
	'U_RECHARGE_ITEM' => append_sid("adr_forge.$phpEx?mode=recharge"),
	'U_CREATE_ITEM' => append_sid("adr_forge.$phpEx?mode=create"),
	'U_MINING' => append_sid("adr_forge.$phpEx?mode=mining"),
	'U_STONE' => append_sid("adr_forge.$phpEx?mode=stone"),
	'U_ENCHANT_ITEM' => append_sid("adr_forge.$phpEx?mode=enchant"),
	'S_FORGE_ACTION'=> append_sid("adr_forge.$phpEx"),
));
