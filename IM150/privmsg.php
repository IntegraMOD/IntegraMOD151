...
$ct_ignorepvar = array('message', 'helpbox', 'delete', 'deleteall');
//-- mod : sub-template ----------------------------------------------------------------------------
//-- add
...
for ($i = 0; $i < count($get_vars); $i++)
{
	...
	if (isset($_GET[$get_vars[$i]])) $s_call .= '&' . $key . '=' . $_GET[$get_vars[$i]];
}
if ( !$newpm )
{
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit && !$error )
	{
		...
		if ( $mode != 'edit' )
		{
			...
			$q = "SELECT *
				  FROM ". ADMIN_PM_TABLE ."
				  WHERE admin_id = '". $to_userdata['user_id'] ."'";
		if (!($r = $db -> sql_query($q)))
			{
		message_die(GENERAL_ERROR, 'Could not query admin pm information.', '', __LINE__, __FILE__, $q);
			}
		$row 			= $db -> sql_fetchrow($r);
		$admin_id 		= $row['admin_id'];
		$admin_redirect = $row['admin_redirect_id'];
		$admin_deny 	= $row['admin_deny'];
		# Check if this admin is deny'ing all prv msg's, allowing admins through though.
		if(($admin_id) && ($userdata['user_level'] != ADMIN) && ($admin_redirect < 1) && ($admin_deny == 1))
			{
		$apmr_deny_msg 	= $board_config['apmr_deny_msg'];
		$show_message 	= str_replace("%U%", $userdata['username'], $apmr_deny_msg);
		message_die(GENERAL_ERROR, $show_message, "Error");
			}
		# Check if this admin is allowing private messages BUT redirecting them to another member.
		if(($admin_id) && ($userdata['user_level'] != ADMIN) && ($admin_redirect > 1) && ($admin_deny == 0))
			{
		$to_userdata['user_id'] = $admin_redirect;
		$show_user_msg 			= "<br><br>". $board_config['apmr_redirect_msg'];	
		$privmsg_subject		= "(APMR) ". $privmsg_subject;		
			}		
