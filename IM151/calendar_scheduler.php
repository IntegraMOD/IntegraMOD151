...
{
redirect(append_sid("login.".$phpEx."?redirect=calendar_scheduler.$phpEx?d=$date&fid=$fid", true));
exit;
} 
...
get_recurring_events($events, $number, $month_start, $month_end, false, 0, -1, $fid);
...
$remaining = $board_config['topics_per_page'] - $displayed;
$local_start = $start-$displayed;
get_recurring_events($events, $topics_count, $start_date, $end_date, true, $local_start, $remaining, $fid, true);
//
...
$template->assign_vars(array(
	...
	'L_TITLE' => $title, // Correct Titles :: Added
	)
);
