 ...
 ***************************************************************************/
    ...
    {
        global $db, $mini_cal_this_year, $mini_cal_this_month, $mini_cal_today;
        ...
        {
            // start and end date
			$start_date = $mini_cal_today;
			user2boardtime($start_date);
			$monthdays = date("t",$start_date) + 1; // add 1 day to go to next month
			$end_date = $start_date + ($monthdays * 24 * 3600);
            ...
            if ( $result = $db->sql_query($sql) )
            {
            	...
            	while( $row = $db->sql_fetchrow($result) )
            	{
					...
					board2usertime($cal_time);
            		$start_day = intval(date('d', $cal_time));
    ...
    function getMiniCalEvents($mini_cal_auth)
    {
        global $template, $db, $phpEx, $lang, $mini_cal_today, 
               $mini_cal_this_month, $mini_cal_this_year, $mini_cal_this_day, $time;
		...
		if (MINI_CAL_DAYS_AHEAD > 0)
        {
            $end_date = $start_date + (MINI_CAL_DAYS_AHEAD * 24 * 3600);
            $days_ahead_sql = " AND topic_calendar_time < $end_date ";
        }
           ...
           if ( $db->sql_numrows($result) > 0 )
           {
       	       ...
       	       while ($row = $db->sql_fetchrow($result))
               {
        			...
        			$template->assign_block_vars('mini_cal_events', array(
                            ...
                            'S_MINI_CAL_EVENT' => smilies_pass($row['topic_title']), 
	...
	function getMiniCalSearchSql($search_date)
	{
		...
		user2boardtime($search_date);
		$nix_tomorrow  = $search_date + (24 * 3600);
