<h1>{L_HEADLINE}</h1>
<p>{L_SUBHEADLINE}</p>

<br />

<div align="center">
<table width="100%" cellspacing="1" cellpadding="3" border="0">
	<tr>
		<td width="48%" align="left" valign="top">
			<table width="100%" cellspacing="1" cellpadding="3" border="0" class="forumline">
				<tr> 
					<th>{L_DONATE}</th>
				</tr>
				<tr> 
					<td class="row2" align="center"><img src="{IMG_ICON_1}" border="0" title="{L_PICTURE_ALT}" alt="{L_PICTURE_ALT}"></td>
				</tr>
				<tr> 
					<td class="row1">
					
					<br />
					
					{L_DONATE_EXPL}

					<br /><br /><br />
					
					<div align="center">
						<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
					  		<input type="hidden" name="cmd" value="_s-xclick">
					  		<input type="image" src="https://www.paypal.com/de_DE/i/btn/x-click-but04.gif" border="0" name="submit" alt="Zahlen Sie mit PayPal - schnell, kostenlos und sicher!">
					  		<img alt="" border="0" src="https://www.paypal.com/de_DE/i/scr/pixel.gif" width="1" height="1">
					  		<input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----">
						</form>
					</div>
					
					<br />
					
					</td>
				</tr>
			</table>
			
			<br />
			
			<table width="100%" cellspacing="1" cellpadding="3" border="0" class="forumline">
				<tr> 
					<th>{L_THANKS_TO}</th>
				</tr>
				<tr> 
					<td class="row2" align="center"><img src="{IMG_ICON_3}" border="0" title="{L_PICTURE_ALT}" alt="{L_PICTURE_ALT}"></td>
				</tr>
				<tr> 
					<td class="row1">
					<br />
					{L_THANKS_TO_TEXT}
					<br /><br /><br /><br />
					<div align="center">
					{L_THANKS_TO_PERS}
					</div>
					<br /><br /></td>
				</tr>
			</table>
			
		</td>
		<td width="4%">&nbsp;</td>
		<td width="48%" valign="top">
			<table width="100%" cellspacing="1" cellpadding="3" border="0" class="forumline">
				<tr> 
					<th>{L_CREDITS}</th>
				</tr>
				<tr> 
					<td class="row2" align="center"><img src="{IMG_ICON_2}" border="0" title="{L_PICTURE_ALT}" alt="{L_PICTURE_ALT}"></td>
				</tr>
				<tr> 
					<td class="row1" align="center">
					<br />
					
					<b>{L_CREDITS_P1}</b><br />
					Christian Knerr (CBACK)
					
					<br /><br /><br /><br />
					
					<b>{L_CREDITS_P2}</b><br />
					<a href="http://www.cback.de" target="_blank">www.cback.de</a><br />
					<a href="http://www.community.cback.de" target="_blank">CBACK Community</a>
					
					<br /><br /><br /><br />
					
					<b>{L_CREDITS_P3}</b><br />
					Crystal Vista XT<br />
					<a href="http://www.paolomod.altervista.org" target="_blank">PaoloMOD</a>
					
					<br /><br /><br /><br />
					
					<b>{L_CREDITS_P4}</b><br />
					<a href="http://www.cback.de" target="_blank">{L_MOD_DOWNLOAD}</a><br />
					<br /><br />
					
					</td>
				</tr>
			</table>
			
			<br />
			
			<table width="100%" cellspacing="1" cellpadding="3" border="0" class="forumline">
				<tr> 
					<th>{L_INFO}</th>
				</tr>
				<tr> 
					<td class="row2" align="center"><img src="{IMG_ICON_4}" border="0" title="{L_PICTURE_ALT}" alt="{L_PICTURE_ALT}"></td>
				</tr>
				<tr> 
					<td class="row1">
					
					<br />
					{L_INFO_TEXT}
					<br /><br />
										
					</td>
				</tr>
			</table>
		</td>
	</tr>
</table>
</div>