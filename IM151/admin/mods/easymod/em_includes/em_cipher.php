 ...
 *
 *   $Id: em_cipher.php,v 1.7 2005/11/05 01:50:32 markus_petrux Exp $
 *
 *
 ***************************************************************************/
...
/*
* THE PURPOSE of this module is purely to encrypt the FTP password.  Since I can't
* be sure what PHP libraries are installed, I cannot use mcrypt.  Thus the addition
* of this 29k file just to make a password safe.  Oh well, EM spares no expense ;-)
*/
 ...
 *
////
//// NOTE: a few small cosmetic changes made by Nuttzy
////
 *
 */
...
class Cipher_blowfish {
     ...
     * @param optional boolean $allparts Return an array containing the two
     *                                   32bit ints also.
     *
...
/**
 * The Cipher_BlockMode_cbc class implements the Cipher Block Chaininng method of
 ...
 *
////
//// NOTE: Some changes by Nuttzy here.  This class is the melding of several classes
////    but I reduced the others down to only what I needed and this is the result.
////
 *
 */
class Cipher_BlockMode_cbc
{
	...
	function encrypt($plaintext, $key)
	{
        ...
        $blocksize = $this->blowfish->getBlockSize();
        ...
        for ($j = 0; $j < $jMax; $j += $blocksize) {
            ...
            $previousCipher = $this->blowfish->encryptBlock($plain);
...
//		$encrypted = str_replace("\n", 'NUTLINE', $encrypted);
		...
		//   to \' and again to ''.  So the SQL string we generate for the DB looks like "single''quote".  But this is
...
//		$ciphertext = str_replace('NUTLINE', "\n", $ciphertext);
        ...
        for ($j = 0; $j < $jMax; $j += $blocksize) {
            ...
            $decrypted .= $this->blowfish->decryptBlock($plain) ^ $previousCipher;
