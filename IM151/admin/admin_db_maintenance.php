		...
		switch($function)
		{
			...
			case 'check_user': // Check user tables
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					echo("<li>" . $row['group_name'] . " (" . $row['group_id'] . ")</li>\n");
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					if ( !($row2 = $db->sql_fetchrow($result2)) ) // No record found
					{
						...
						echo("<li>" . $row['group_name'] . " (" . $row['group_id'] . ") - " . $lang['Moderator_added'] . "</li>\n");
					...
					elseif ( $row2['user_pending'] == 1 ) // Record found but moderator is pending
					{
						...
						echo("<li>" . $row['group_name'] . " (" . $row['group_id'] . ") - " . $lang['Moderator_changed_pending'] . "</li>\n");
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					echo("<li>" . $row['username'] . " (" . $row['user_id'] . ")</li>\n");
					$result_array[] = $row['user_id'];
				}
				$db->sql_freeresult($result);
				if ($list_open)
			...
			case 'check_post': // Checks post data
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					echo("<li>" . $row['forum_name'] . " (" . $row['forum_id'] . ")</li>\n");
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					echo("<li>" . sprintf($lang['Deleting_post_wo_text'], $row['post_id'], htmlspecialchars($row['topic_title']), $row['topic_id'], $row['username'], $row['user_id']) . "</li>\n");
					$result_array[] = $row['post_id'];
				}
				$db->sql_freeresult($result);
				if ($list_open)
					...
					if ( $current_topic != $row['topic_id'] || !$row )
					{
						if ( count($result_array) )
						{
							...
							$sql2 = 'INSERT INTO ' . TOPICS_TABLE . " (forum_id, topic_title, topic_poster, topic_time, topic_views, topic_replies, topic_status, topic_vote, topic_type, topic_first_post_id, topic_last_post_id, topic_moved_id)
								VALUES ($new_forum, '$topic_title', " . $row2['poster_id'] . ", " . $row2['post_time'] . ", 0, $post_replies, " . TOPIC_UNLOCKED . ", 0, " . POST_NORMAL . ", $first_post, $last_post, 0)";
							$result2 = $db->sql_query($sql2);
							if ( !$result2 )
							{
							...
							echo("<li>" . sprintf($lang['Setting_topic'], $record_list, $topic_title, $new_topic, $lang['New_forum_name']) . " </li>\n");
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					echo("<li>" . sprintf($lang['Setting_post_forum'], $row['post_id'], $row['p_forum_name'], $row['p_forum_id'], $row['t_forum_name'], $row['t_forum_id']) . "</li>\n");
					$sql2 = "UPDATE " . POSTS_TABLE . "
			...
			case 'check_pm': // Check private messages
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					echo("<li>" . sprintf($lang['Deleting_pn_wo_text'], $row['privmsgs_id'], htmlspecialchars($row['privmsgs_subject']), $row['from_username'], $row['from_user_id'], $row['to_username'], $row['to_user_id']) . "</li>\n");
				...
				for($i = 0; $i < count($tables); $i++)
				{
					...
					while ( $row = $db->sql_fetchrow($result) )
					{
						...
						else //  We got an error
						{
							...
							{
								echo("<li><b>$tablename:</b> " . $row['Msg_text'] . "</li>\n");
							}
							$db->sql_freeresult($result2);
						}
					}
