...
// END
		...
		if(!empty($cat_id))
		{
			if(!$this->auth[$cat_id]['auth_upload'])
			{
				...
				{
					redirect(append_sid("login.$phpEx?redirect=dload.$phpEx?action=user_upload&cat_id=" . $cat_id, true));
				}
		...
		if ( isset($_POST['submit']) )
		{
			if(!$file_id)
			{
				...
				{
					$message = $lang['Fileadded'] . '<br /><br />' . sprintf($lang['Click_return'], '<a href="' . append_sid("dload.php?action=file&file_id=" . $temp_id) . '">', '</a>');
				}
				else
				{
					$message = $lang['Fileadded_not_validated'] . '<br /><br />' . sprintf($lang['Click_return'], '<a href="' . append_sid("dload.php?action=category&cat_id=" . $cat_id) . '">', '</a>');
				}
//				$mode = 'edit';
			}
			elseif($file_id != '')
			{
				...
				$this->_pafiledb();
				$message = $lang['Fileedited'] . '<br /><br />' . sprintf($lang['Click_return'], '<a href="' . append_sid("dload.php?action=file&file_id=" . $file_id) . '">', '</a>');
//				$mode = 'edit';
			}
