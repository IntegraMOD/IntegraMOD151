...
class pafiledb_Template
{
	...
	function _tpl_load(&$handle)
	{
		...
		{
			message_die(GENERAL_ERROR, "Pafiledb Template->loadfile(): No file specified for handle $handle");
		}
		...
		{
			message_die(GENERAL_ERROR, "Pafiledb Template->_tpl_load(): File $filename does not exist or is empty");
		}
	...
	function compile_var_tags(&$text_blocks)
	{
		...
		else
		{
			...
			$text_blocks = preg_replace_callback('#\{L_([A-Z0-9\-_]*?)\}#', function ($matches) {
        return '<?php echo (isset(\$this->_tpldata[\'.\'][0][\'L_\\1\']) ? \$this->_tpldata[\'.\'][0][\'L_' . $matches[1] . '\'] : (isset($lang[\'' . $matches[1] . '\']) ? $lang[\'' . $matches[1] . '\'] : "")); ?>';
			}, $text_blocks);
		}
